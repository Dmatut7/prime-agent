...
3. **测试环境必须清污**：`env -u RLM_DEPTH -u RLM_SESSION_DIR -u RLM_MAX_DEPTH -u FORCE_COLOR -u PRIME_AGENT_INTERNAL_DAEMON_WORKER`。否则假红/假绿。
4. **后台任务配看门狗**：日志 mtime 5 分钟不动判死，杀掉重来。禁止前台 sleep 干等。
5. **配置/文档声明 ≠ 运行时事实**。本轮已抓 5 例：README 表格 1 条已过期、#1955 commit message 声称加了测试实际零测试文件、`system-prompt.ts:14` 注释声称 toolSnippets 用于 custom prompts 实际从不读取、lane A 说 #1957 需迁移 12 处调用实际本地一行未改、**v1 任务书自己说「不新增依赖」实际官方新增了 `grok-mermaid@0.2.3`**。
...
| **`omitStreamingMessages`** | **【v2.2 再修正】worker↔supervisor 半边改造后保留；客户端半边**只剩 `[MT] agents-view-mode.ts:1908` 一处（删除确认期 liveness 查询，**块外存活**），`:2164`/`:2495` 随块 3/块 4 取官方而消失 → **`createAgentsViewListCommand` 保留 client 参数与 omit 逻辑**（对 `:1908` 仍有效），只删已死的调用点。**v2 曾写「客户端半边已成死代码」是错的**（rev2-facts R4：`:1908` 在任何解法下都活着，照"死代码"退回 0 参会让这次 list 带整段 in-flight assistant message） | **v1 判「丢，取官方」是错的，rev-facts BL2 三条反证 + rev-exec I5 独立复核，根代理已亲手验证**：① #1909(`15ef45668`) 是修 reconnect-loop 的，其 diff 里 `streamingMessage` **命中 0**；做 Omit 的是 **#1897**，Omit 的对象是 `RosterSessionSummary`（**roster wire，不是 list**）② 官方 `[UP] daemon-session-list.ts:68` `streamingMessage?: AgentMessage;`、`:270` `streamingMessage: session.state.streamingMessage,` → **官方 list 照旧带整段助手消息** ③ `omitStreamingMessages`/`summaryWithoutStreamingMessage`/`list_without_streaming_messages` 在 `[UP]` 全仓 **0 命中**，`[HEAD]` 有 **12 处**（agents-view-mode / daemon-mode / daemon-protocol / daemon-session-list / daemon-supervisor）④ `[UP] daemon-supervisor.ts:4088` 仍是裸 `pullSource.request({ type: "list" }, 5000)`。**照 v1 做会静默删掉一个官方零覆盖的本地独有优化。** |
...
- **S-2 备案（记 docs/fork/ 本地轴清单）**：`refreshWorkerSummaries` 第 5 位 `omitStreamingMessages` 是**全休眠接缝**——src 12 处调用无一传第 5 实参（`:4174` 只是内部 retry 透传），唯一撑着它的是 `daemon-supervisor-streaming-list.test.ts`。将来任何「删无用参数」清理会连这条本地轴一起删。活路径仍在：`agents-view-mode.ts:229` 发 → `daemon-supervisor.ts:2734` 剥 / `daemon-mode.ts:3899` 剥。
...
- ~~P0-A 修法~~ → **v1 与两个审查代理的方案都被否，采 rev-exec §5.3 的三处联动**（S2.1）
