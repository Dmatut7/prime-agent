...
- Changed the daemon to rewrite a worker descriptor only when its durable content moved, so routine session list polls no longer rewrite every descriptor once per poll.
