...
interface TemporarySkillHost {
	replaceTemporarySkills(skillPaths: string[], source: string): Promise<void> | void;
	...
	async configure(servers: readonly McpServer[], cwd: string): Promise<void> {
		...
		for (const [server, spec] of Object.entries(config)) {
			...
			for (const tool of tools) {
				...
				const skillName = skillDirectoryName(importName);
				if (!/^[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?$/.test(skillName)) {
					throw RequestError.invalidParams({
						reason: `MCP tool ${server}/${tool.name} produces invalid Python skill name ${skillName}`,
					});
				}
		...
		try {
			await this.host.replaceTemporarySkills(skillPaths, "acp:mcp");
		} catch (error) {
